  ...
  while (!quit)    {
    while(SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_KEYDOWN) {
        ...
        on_key_press(keyPressed); //pass the key to callback        
      }
...
int RendererManager::quit_now(int out){
  quit = true;
  return out;
}
...
void RendererManager::handleCustomEvent(const SDL_Event& event) {
  ...
  if (event.user.code == kRendererCreate) {
    if(event.user.data2 == NULL){
      std::string window_name = "Renderer " + std::to_string(event.user.timestamp);
      renderers[event.user.data1] = std::make_shared<Renderer>(window_name.c_str());
    }else
      renderers[event.user.data1] = std::make_shared<Renderer>((char*)event.user.data2);
  }
...
void RendererManager::createNamedRenderer(void* key, void* name) {  
  pushEvent(kRendererCreate, key, name);
}
...
//   assert(event.type == event_type_);
//   if (event.user.code == kRendererCreate) {
//     std::cout << "CreateRenderer with name: " << (char*)event.user.data2 << std::endl;
//     // std::string window_name = "Window" + std::string((char*)event.user.data2);
//     // renderers[event.user.data1] = std::make_shared<Renderer>(window_name.c_str());
//     renderers[event.user.data1] = std::make_shared<Renderer>((char*)event.user.data2);
//   }
//   if (event.user.code == kRendererDraw) {
